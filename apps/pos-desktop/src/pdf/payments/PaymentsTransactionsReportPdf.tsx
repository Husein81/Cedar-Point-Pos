...
export const PaymentsTransactionsReportPdf: React.FC<PaymentsTransactionsReportProps> = ({
    tenantName,
    branchName,
    dateRange,
    summary,
    ...
    const tableColumns = [
        ...
        { header: 'Amount', accessor: 'amount', width: '10%', align: 'right' as const, format: formatCurrency },
            ...
            {summary.byMethod.length > 0 && (
                <View style={{ marginBottom: 12 }}>
                    ...
                    {summary.byMethod.map((item, idx) => (
                        <View key={idx} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4, paddingHorizontal: 8 }}>
                            <Text style={{ fontSize: 10, width: '40%' }}>{item.method}</Text>
                            <Text style={{ fontSize: 10, width: '30%' }}>{item.count} transactions</Text>
                            <Text style={{ fontSize: 10, width: '30%', textAlign: 'right' }}>{formatCurrency(item.amount)}</Text>
                        </View>
                    ))}
                </View>
            )}
