...
export const InlineKeypad = () => {
  ...
  const handlePaymentConfirm = async (payments: PaymentEntry[]) => {
    ...
    try {
      ...
      // Capture tableId before clearing the order
      const tableIdToReset = order.tableId;
      ...
      if (result.status === OrderStatus.COMPLETED) {
        ...
        // Reset table status to AVAILABLE after successful payment
        if (tableIdToReset) {
          updateTableStatus.mutate({ id: tableIdToReset, status: "AVAILABLE" });
        }
      }
    } catch (error) {
