...
import { Button, Icon, Input, Badge, Shad, cn } from "@repo/ui";
...
const STATUS_CONFIG: Record<TableStatus, { label: string; color: string; bgClass: string; icon: string }> = {
    AVAILABLE: {
        label: "Available",
...
export function TableSelectorModal({ onTableSelect, currentTableId }: TableSelectorModalProps) {
    ...
    const handleSelect = useCallback((table: TableWithFloor) => {
        onTableSelect(table);
        closeModal();
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <Shad.DialogTitle className="text-lg font-semibold">Select Table</Shad.DialogTitle>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-16">
                    <Icon name="AlertCircle" className="w-10 h-10 text-destructive mx-auto mb-2" />
                    <p className="text-sm text-muted-foreground">Failed to load tables</p>
                </div>
            ) : filteredTables.length === 0 ? (
                <div className="text-center py-16">
                    <Icon name="Search" className="w-10 h-10 text-muted-foreground mx-auto mb-2" />
                                ...
                                {floorTables.map((table) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {/* Status Badge */}
                                            <Badge variant="outline" className={cn("mt-1.5 text-xs", config.color)}>
                                                {config.label}
                                            </Badge>
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t">
                {currentTableId ? (
                    <Button variant="ghost" size="sm" onClick={handleClearSelection} className="text-destructive">
