...
export const FinancialsReportPdf: React.FC<FinancialsReportProps> = ({
  ...
  const bestSellersColumns = [
    { header: "Product", accessor: "productName", width: "50%" },
    {
      ...
      width: "25%",
      align: "right" as const,
      format: (val: number) => (val ? val.toString() : "-"),
    },
    {
      header: "Revenue",
      accessor: "revenue",
  ...
  return (
    ...
    >
      ...
      <PdfTable data={topProfitProducts} columns={profitProductsColumns} />

      {/* Best Sellers */}
      <Text style={styles.sectionTitle}>Top 5 Best Sellers</Text>
      <PdfTable data={bestSellers} columns={bestSellersColumns} />

      {/* Low Performance Products */}
      <Text style={styles.sectionTitle}>Low Performance Products</Text>
      <PdfTable data={lowPerformance} columns={bestSellersColumns} />

      {/* Revenue by Category */}
      <Text style={styles.sectionTitle}>Revenue by Category</Text>
      <PdfTable data={categories} columns={categoryColumns} />
    </PdfDocument>
  );
};
