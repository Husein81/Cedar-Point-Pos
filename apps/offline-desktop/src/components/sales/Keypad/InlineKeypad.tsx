...
export const InlineKeypad = () => {
  ...
  const { setDiscount, updateItemNotes, order, subtotal } = useOrderStore(
    useShallow((s) => ({
  ...
  const resolveDiscountMode = useCallback(
    (ctx: KeypadContext): DiscountType => {
      if (ctx === "DISCOUNT_FIXED") return DiscountType.FIXED;
  ...
  const applyKeypadValue = useCallback(
    (numericValue: number) => {
      ...
      if (safeContext === "DISCOUNT_PERCENT" || safeContext === "DISCOUNT_FIXED") {
  ...
  const isDiscountContext =
    safeContext === "DISCOUNT_PERCENT" || safeContext === "DISCOUNT_FIXED";
  ...
  const openDiscountForOrder = useCallback(
    (discountContext: "DISCOUNT_PERCENT" | "DISCOUNT_FIXED") => {
      ...
      const discountTypeValue =
        discountContext === "DISCOUNT_PERCENT"
  ...
  const title = useMemo(() => {
    ...
    if (isDiscountContext) {
      return activeDiscountMode === DiscountType.FIXED
        ? "Order Discount ($)"
        : "Order Discount (%)";
    }
